...
20–96 可設定、4 個背景主題（default/warm/cool/dark）、9 名角色個別客製化
（名稱/個性/人設/回覆方式 reply_style/llm_profile L1-L3 覆寫）、攝影機
縮放平移、fixture 模式存檔持久化（data/world_save.json，重啟留存）。
經雙鏡頭對抗審查（Rust 併發×前端合約）發現 2 Critical＋4 Major 全數修復；
workspace 測試 173 個全綠＋web build/lint 綠＋端到端瀏覽器實測
（房間 60×40+warm 套用/422 防呆/角色 PATCH/重啟持久化）。

Phase 1（辦公室世界與渲染，T1.1–T1.5）於 `phase/1-world`：
9 名 agent 依附錄 A.2 於 07:00–09:00 從大門進場走到自己座位坐下；
前端 PixiJS 即時渲染＋時間控制（P1 驗收後 124 測試基準）。
...
回歸掃描發現上述修復本身引入的 2 條 minor，合併 main 前清除：

- **回歸 #1 — `OfficeCanvas` 載入失敗殘留 in-flight promise**：Major 修法
  把 in-flight `Promise<AgentVisual>` 存進 visuals map，但 `Assets.load`
  失敗（如暫時性 404）時，rejected 的 promise 會被永久快取，該 agent 到
  重整前都不再有 visual；`syncAgents` 的 `await ensureVisual` 會 throw 中斷
  整個迴圈，排在後面的 agent 收不到位置更新；`void syncAgents`／清理迴圈的
  `void pending.then` 皆無 `.catch`→unhandled rejection。修法：①`ensureVisual`
  失敗時把該筆從 visuals map 移除（下次 sync 自癒重建）並 re-throw；
  ②`syncAgents` 對每個 agent 的 `ensureVisual` 以 try/catch 包裹、單一失敗
  `continue` 不影響其他 agent；③兩處 fire-and-forget 補 `.catch`。原 Major
  的並發不重複 addChild 不變式保持不變（`visuals.set` 仍在 await 前同步登記，
  rejected build 從未 addChild）。
- **回歸 #2 — `stuck_in_place` 借用 `agent_status` 通道造成前後端分歧**：
  server 端 `current_status` 保持 `walking`（world.rs 測試明確斷言），但
  loud stuck 訊號原本以 `AgentStatus{status:"stuck_in_place"}` 送出，web store
  照單全收寫入 `current_status`，client 端變 `stuck_in_place`→`OfficeCanvas`
  的 `v.status === "walking"` 動畫閘門停掉走路動畫，且 server 之後成功 reroute
  也不再送 walking 校正，直到 seated 才自癒。修法（準則：client 永不渲染 server
  不持有的狀態）：新增獨立事件型別 `SimEvent::AgentStuck{agent_id}`
  （序列化為 `agent_stuck`、**不帶 status 欄位**），world.rs 改發此事件、
  `current_status` 完全不動；前端新增 `AgentStuckMsg` 型別，store 收到只
  `console.warn`、不改任何 rendered state。斷言佐證：events.rs 測 `agent_stuck`
  無 `status` 欄；world.rs 的 stuck streak 測試改比對 `AgentStuck`，並加守衛
  斷言任何 `AgentStatus` 都不會帶 `stuck_in_place`（前後端不再分歧）。

## Phase 3 前置重大決策（2026-07-11/12，ADR-002 摘要）

- **單一產圖核心**：`web/src/lib/office_shell_core.mjs`（純 ESM 零 Node API，
  瀏覽器與 scripts/ 共用）；`gen_office_shell.mjs` 為薄殼 CLI
  （--width/--height/--theme/--out）。預設參數輸出與既有檔 byte-identical
  （ci.sh 決定性層驗證）。禁止出現第二份產圖邏輯。
- **世界配置 REST**：GET/PUT `/api/v1/world/map`（驗證 20..=96/門/flood-fill/
  家具在界，422 附最小可行尺寸繁中提示）、PUT `/api/v1/world/layout`、
  PATCH `/api/v1/agents/:id`（llm_profile 僅 L1-L3、拒 L0）。兩個 PUT 成功
  即世界重置 07:00 paused 並廣播全量 snapshot（world.map_rev 遞增為前端
  重抓地圖的訊號）；PATCH 不重置。錯誤一律 `{"error":{"code","message"}}`。
- **持久化**：fixture 模式原子寫 `WORLD_SAVE_PATH`（預設 repo/data/
  world_save.json，gitignored），開機優先載入、損壞 WARN 忽略；載入時
  map+layout 成對驗證（`replace_map_and_layout`，2026-07-12 審查修復——
  先前兩步式載入會把「縮房+緊湊佈局」合法存檔整包丟棄）。db 模式寫
  `worlds.map_tmj`／layout delete+reinsert 後**還原 agents.desk_id**
  （FK ON DELETE SET NULL 級聯對策，未經真實 PG 驗證）。
- **編輯器狀態隔離**：編輯旗標為 store 本地欄位 `editorActive`＋
  `activeTab`（分頁感知），**嚴禁挪用 `world.status`** 之類伺服器欄位當
  前端旗標（審查 Critical 教訓：ws snapshot 全量覆蓋會清掉草稿）。
- **reply_style 全鏈路**：`agents.reply_style` 欄位（001_init.sql）→ seed
  9 人預設值 → fixture → ws snapshot → 前端編輯 → `prompts/converse.md`
  的 `{{reply_style}}` 佔位符（Phase 2 管線實作後自動生效）。
  「不得硬編碼服從」紅線與 rank_relation 行原樣保留。
- **攝影機**：viewport 固定 48×32 tile 基準；最小縮放動態
  `min(0.5, fitZoom)` 使 96×96 也能「適配」入鏡。

## 已知缺口（Known Gaps）

- **LimeZu 付費素材包仍未落地本機**（2026-07-11 全機掃描零命中）：管線已
  就緒（`sync_limezu_assets.mjs`＋`manifest.example.json`＋編輯器缺包指引
  橫幅），使用者把素材放入 `assets/tilesets/limezu-modern-office/` 並跑
  `node scripts/sync_limezu_assets.mjs` 即可點亮真實家具圖。目前素材庫為
  自製 CC0 色塊佔位＋4 背景主題。
- **座位指派（agent↔desk）僅本地預覽**：PATCH /agents 合約不含 desk_id，
  PUT /world/layout 由 agent 自身 desk_id 解析座位——編輯器的「座位指派」
  下拉不會持久化（程式內已註解）。要做需擴充 PATCH 合約＋世界重置語意
  設計，列 Phase 3 正式範圍。
- **db 模式 desk_id 還原（db_persist.rs remap）未經真實 Postgres 驗證**：
  本機無 Docker/psql；純邏輯有單元測試，事務/FK 行為第一次在 compose
  環境跑時必驗（沿用既有「DB 路徑未實機驗證」缺口條目的驗證清單）。
